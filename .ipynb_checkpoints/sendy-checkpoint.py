...
# AWS s3 bucket
data = spark.read.csv('s3://sendylogistic/Train(1).csv', inferSchema=True,header=True)
rider = spark.read.csv('s3://sendylogistic/Riders.csv', inferSchema=True,header=True)

# GCP cloud storage
data = spark.read.csv('gs://sendylogistic/Train(1).csv', inferSchema=True,header=True)
rider = spark.read.csv('gs://sendylogistic/Riders.csv', inferSchema=True,header=True)
print("=====Data loaded=====")
...
#GCP cloud storage
pipeline_model.save("gs://sendylogistic/Saved_pipeline/v1")
